...
class Gotchipus(Base):
    ...
    async def mint_wearable(self):
        ...
        logger.debug(
            f"{self.wallet} | {self.__module__} | trying to mint Wearable")
    ...
    async def transfer_from_gotchipus(self):
        ...
        amount = TokenAmount(
            amount=randfloat(from_=0.00001, to_=0.001, step=0.00001)
        )
    ...
    async def can_check_pet(self):
        ...
        if (pet_check_time + PET_COOLDOWN - now) < 0: return True
        return False
