...
class PharosPortal(Base):
    ...
    async def bind_twitter(self, redirect_url: str):
        ...
        payload = {**query, "address": self.client.account.address}
    ...
    async def get_faucet_status(self) -> dict:
        ...
        r = await self.session.get(url=f"{self.BASE}/faucet/status", headers=headers, params=params)
    ...
    async def follow_and_verify_twitter_task(self, twitter_tasks: list[dict]) -> list[dict]:
        ...
        for task in twitter_tasks:
            ...
            payload = {"address": self.client.account.address, "task_id": task_id}

            r = await self.session.post(
                url=f"{self.BASE}/task/follow" if task_type == "Follow" else f"{self.BASE}/task/verify", headers=headers, json=payload
            )
    ...
    async def verify_task(self, task: dict) -> dict:
        ...
        payload = {"address": self.client.account.address, "task_id": task["task_id"]}

        r = await self.session.post(url=f"{self.BASE}/task/verify", headers=headers, json=payload)
        r.raise_for_status()
    ...
    async def get_discord_oauth_code(self):
        ...
        r = await self.session.get(url=f"{self.BASE}/auth/discord", headers=headers, allow_redirects=False)
        r.raise_for_status()
    ...
    async def bind_discord(self, url, state):
        ...
        r = await self.session.post(url=f"{self.BASE}/auth/bind/discord", headers=headers, json=payload)
        r.raise_for_status()
