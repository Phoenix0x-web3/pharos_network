...
class Zenith(Base):
    ...
    async def swap_to_r2_usdc(self):
        ...
        return await self._swap(from_token=Contracts.PHRS, to_token=USDC_R2, amount=TokenAmount(amount=amount), slippage=30)

    async def correct_tokens_position(self, from_token: RawContract, to_token: RawContract, a_amt: TokenAmount, b_amt: TokenAmount = None):
    ...
    async def get_pool_address(self, from_token: RawContract, to_token: RawContract, fee: int = 500):
        ...
        data = TxArgs(tokenA=from_token.address, tokenB=to_token.address, fee=fee).tuple()
        ...
        try:
            ...
            if pool_address == "0x0000000000000000000000000000000000000000":
                ...
                return RawContract(title="POOL", address=pool_address, abi=read_json(path=(ABIS_DIR, "zenith_v3_pool.json")))
        except Exception as e:
            logger.exception(e)
            return None

    async def get_price_pool(
        self, from_token: RawContract, to_token: RawContract, a_amt: TokenAmount, b_amt: TokenAmount = None, fee: int = 500
    ):
        from_token, to_token, a_amt, b_amt = await self.correct_tokens_position(
            from_token=from_token, to_token=to_token, a_amt=a_amt, b_amt=b_amt
        )
    ...
    async def _swap(self, amount: TokenAmount, from_token: RawContract, to_token: RawContract, slippage: float = 3.0, fee: int = 500):
        ...
        if not to_token_is_phrs:
            amount_out_min = TokenAmount(
                amount=amount_out_min.Ether, decimals=await self.client.transactions.get_decimals(contract=from_token.address)
            )
        ...
        if from_token_is_phrs:
            ...
            second_item = contract.encode_abi("unwrapWETH9", args=[amount_out_min.Wei, self.client.account.address])
        else:
            second_item = None

        encode = contract.encode_abi("multicall", args=[deadline, [item for item in [encode, second_item] if item is not None]])

        if not from_token_is_phrs:
            if await self.approve_interface(token_address=from_token.address, spender=contract.address, amount=None):
        ...
        if receipt:
            return f"Success swap {amount.Ether:.5f} {from_token.title} to {amount_out_min.Ether:.5f} {to_token.title}"
    ...
    async def zenith_faucet(self):
        ...
        captcha_task = await capmoster.get_recaptcha_task_turnstile(websiteKey=site_key, websiteURL=web_site, cdata=cdata)
...
class ZenithLiquidity(Zenith):
    ...
    async def liquidity_controller(self):
        ...
        return await self.prepare_position(a_token=from_token, b_token=to_token, amount=a_amt, fee=fee)

    async def process_back_swap_from_natve(self, token: RawContract, amount: TokenAmount, fee: int = 3000):
        from_token = Contracts.WPHRS
        to_token = token
        ...
        logger.debug(f"{self.wallet} | {self.__module_name__} | going to swap {amount} {from_token.title} to {to_token.title}")

        return await self._swap(amount=amount, from_token=Contracts.PHRS, to_token=token, fee=fee)

    async def prepare_position(self, a_token: RawContract, b_token: RawContract, amount: TokenAmount, fee: int = 500):
        from_token_is_phrs = a_token.address.upper() == Contracts.PHRS.address.upper()
        if from_token_is_phrs:
            a_token = Contracts.WPHRS

        to_token_is_phrs = b_token.address.upper() == Contracts.PHRS.address.upper()
        if to_token_is_phrs:
            b_token = Contracts.WPHRS
        ...
        if await self._is_same_token(a_token, from_token):
            amt0 = TokenAmount(
                amount=float(amount.Ether), decimals=await self.client.transactions.get_decimals(contract=from_token.address)
            )
            amt1 = TokenAmount(
                amount=float(amt0.Ether) * float(price), decimals=await self.client.transactions.get_decimals(contract=to_token.address)
            )
        elif await self._is_same_token(a_token, to_token):
            amt1 = TokenAmount(amount=float(amount.Ether), decimals=await self.client.transactions.get_decimals(contract=to_token.address))
            amt0 = TokenAmount(
                amount=float(amt1.Ether) / float(price), decimals=await self.client.transactions.get_decimals(contract=from_token.address)
            )
        else:
            raise RuntimeError()
        ...
        logger.debug(f"{self.wallet} | {self.__module_name__} | prepare LP {fee} | {amt0} {from_token.title} <-> {amt1} {to_token.title}")
        ...
        return await self.add_liquidity(from_token=from_token, to_token=to_token, a_amt=amt0, b_amt=amt1, fee=fee)
    ...
    async def add_liquidity(
        ...
        if slippage:
            a_amt_min = TokenAmount(amount=float(a_amt.Ether) * (100 - slippage) / 100, decimals=a_amt.decimals)
            b_amt_min = TokenAmount(amount=float(b_amt.Ether) * (100 - slippage) / 100, decimals=b_amt.decimals)
        else:
            a_amt_min = 0
            b_amt_min = 0

        logger.debug(
            f"{self.wallet} | {self.__module_name__} | add LP {fee} | {a_amt} {from_token.title}, min {a_amt_min} "
            f"-- {b_amt} {to_token.title}, min {b_amt_min}"
        )

        for token, amt in ((from_token, a_amt), (to_token, b_amt)):
            if token != Contracts.WPHRS:
                ok = await self.approve_interface(token_address=token.address, spender=c.address, amount=None)
                if not ok:
        ...
        current_position = [
            cur for cur in current_position if cur.get("from_token") == from_token.address and cur.get("to_token") == to_token.address
        ]
        ...
        tx = await self.client.transactions.sign_and_send(TxParams(to=c.address, data=data, value=value.Wei))
    ...
    async def descrease_liquidity(self, pos, slippage=3):
        ...
        descrease = TxArgs(tokenId=token_id, liquidity=liquidity, amount0Min=amount0Min, amount1Min=amount1Min, deadline=deadline).tuple()
        ...
        collect = TxArgs(
            tokenId=token_id, recipient=self.client.account.address, amount0Requested=max_uint128, amount1Requested=max_uint128
        ).tuple()
